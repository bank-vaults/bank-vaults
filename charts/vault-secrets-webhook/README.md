...
**In case of the K8s version is lower than 1.15 the namespace where you install the webhook must have a label of `name` with the namespace name as the value, so the `namespaceSelector` in the `MutatingWebhookConfiguration` can skip the namespace of the webhook, so no self-mutation takes place. If the K8s version is 1.15 at least, the default `objectSelector` will act and prevents the self-mutation**
...
| env.VAULT_IMAGE                  | vault image                                                                  | `vault:1.4.0`                      |
| env.VAULT_ENV_IMAGE              | vault-env image                                                           | `banzaicloud/vault-env:1.2.0`      |
